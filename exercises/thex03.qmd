...
```{r}
pacman::p_load(readxl, sf, tidyverse, tmap, sfdep, gifski, httr, jsonlite, onemapsgapi, rvest, ggpubr, olsrr, GWmodel, SpatialML, Metrics, ggplot2, plotly)
```
...
```{r}
ggplot(data = resale.rftest.tmap,
       aes(x = error, y = "")) +
  geom_boxplot()
```
...
## Comparative Analysis of Errors by Towns
...
For reference, the interactive map on the left is the results from OLS Multiple Linear Regression Model and the right is from the Goegraphically Weighted Random Forest.
...
```{r}
generate_rmse_town <- function(geo1, geo2, AREA) {
  plot_map_t1 <- filter(geo1, town == AREA)
  plot_map_t2 <- filter(geo2, town == AREA)
  t1r <- rmse(plot_map_t1$resale_price, plot_map_t1$predict)
  t2r <- rmse(plot_map_t2$resale_price, plot_map_t2$GRF_pred)
  print("RMSE FOR: ")
  print(AREA)
  print("==============")
  print("RMSE OLS:")
...
```{r}
#| code-fold: true
generate_rmse_town(resale.mlrtest, test_data_p, "JURONG WEST")
```
...
```{r}
#| code-fold: true
generate_rmse_town(resale.mlrtest, test_data_p, "WOODLANDS")
```
...
```{r}
#| code-fold: true
generate_rmse_town(resale.mlrtest, test_data_p, "PUNGGOL")
```
...
```{r}
#| code-fold: true
generate_rmse_town(resale.mlrtest, test_data_p, "TAMPINES")
```
...
```{r}
#| code-fold: true
generate_rmse_town(resale.mlrtest, test_data_p, "BUKIT MERAH")
```
...
Accesbility metrics may be useful instead of distance to regions - less acccurate prediction vs network
