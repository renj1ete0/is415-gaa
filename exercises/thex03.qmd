...
```{r eval=FALSE}
coords_hipmup <- get_coordinates(addr_lst.hipmup)
```
...
write_rds(coords_hipmup, "Take-Home_Ex03/rds/coords_hipmup.rds")
...
coords_hipmup <- read_rds("Take-Home_Ex03/rds/coords_hipmup.rds")
...
1.  Merge Coordinate Data and Original Dataframe
    ...
    ```{r}
    temp_hdb_hip_mup <- left_join(hdb_hip_mup_trans, coords_hipmup, by=c("address" = "address"))
    ```

2.  Check for `NA` X/Y values and manually amend if required

    ```{r}
    filter(temp_hdb_hip_mup, is.na(x) == TRUE)
    ```
    ...
    | HDB Address        | Issue                                                             |
    +====================+===================================================================+
    | 5 BANDA ST         | OneMap recognises it as 5 BANDA STREET, we will amend accordingly |
    +--------------------+-------------------------------------------------------------------+
    | 116 ALJUNIED AVE 4 | Address is **116 ALJUNIED AVE 2**, we will amend accordingly      |
    +--------------------+-------------------------------------------------------------------+
    | 119 ALJUNIED AVE 5 | Address is **119 ALJUNIED AVE 2**, we will amend accordingly      |
    +--------------------+-------------------------------------------------------------------+
    ...
    We use *mutate()* and *ifelse()* condition to update the Postal Codes of the clinics at the relavant addresses.

    ```{r}
    hdb_hip_mup_trans_upd <- hdb_hip_mup_trans %>% 
      mutate(address = ifelse(address == "5 BANDA ST", "5 BANDA STREET", address)) %>%
      mutate(STREET = ifelse(STREET == "BANDA ST", "BANDA STREET", STREET)) %>%
      mutate(STREET = ifelse(STREET == "ALJUNIED AVE 4" & BLK == "116", "ALJUNIED AVE 4", STREET)) %>%
      mutate(address = ifelse(address == "116 ALJUNIED AVE 4", "116 ALJUNIED AVE 2", address)) %>%
      mutate(address = ifelse(address == "119 ALJUNIED AVE 5", "119 ALJUNIED AVE 2", address)) %>%
      mutate(street_name = ifelse(STREET == "ALJUNIED AVE 5" & BLK == "119", "ALJUNIED AVE 2", STREET))
    ```

    Lastly, we regenerate the list of unique Postal Codes to be geocoded.

    ```{r}
    addr_lst.hipmup_upd <- sort(unique(hdb_hip_mup_trans_upd$address))
    glimpse(addr_lst.hipmup_upd)
    ```
    ...
    ```{r eval=FALSE}
    coords_hipmup_upd <- get_coordinates(addr_lst.hipmup_upd)
    ```

    Saving the DataFrame as .rds for future use to prevent rerunning *get_coordinates()* GET API everytime a render is run

    ```{r eval=FALSE}
    write_rds(coords_hipmup_upd, "Take-Home_Ex03/rds/coords_hipmup_upd.rds")
    ...
    ```{r}
    coords_hipmup_upd <- read_rds("Take-Home_Ex03/rds/coords_hipmup_upd.rds")
    ...
    ```{r}
    temp_hdb_hip_mup <- left_join(hdb_hip_mup_trans_upd, coords_hipmup_upd, by=c("address" = "address"))
    filter(temp_hdb_hip_mup, is.na(x) == TRUE)
    ```
...
3.  Convert a DataFrame into a sf Object
    ...
    ```{r}
    hdb_hip_mup_sf <- st_as_sf(temp_hdb_hip_mup,
                            coords = c("x", "y"),
                            crs = 3414)
    ```
...
4.  Plot a tmap and check if points are plotted in the correct regions
    ...
    ```{r}
    tmap_mode("view")
    tm_shape(hdb_hip_mup_sf) +
      tm_dots("TYPE",
              popup.vars=c("BLK"="BLK", "STREET"="STREET", "TYPE" = "TYPE", "TOWN" = "TOWN"))
    ```
