...
Keep 9 column cos impt bla bla and rest for proc
...
Now, let us try to identify how to identify and remove the outer islands. Below, we will plot `KAB_KOTA` which is the City District.
...
::: {.callout-note icon="false"}
Recall that the filter function will drop NA rows automatically. Hence, we are expecting 260 rows per month, for a total of 3130 rows for the whole 12 months
:::
...
Yes, the, the data has been processed correctly with 3132 rows which was calculated earlier.
...
```{r}
tmap_animation(vacrate_anim, filename = "Take-Home_Ex02/vacrate.gif", delay = 100, width = 1280, height = 720)
```
